...
try {
    ...
    $sql = "select fname, co.i_autocode, concat(co.i_autocode, imf.s_fileformat) newfile, rf.id rfid
            from restore_files rf, restore_file_co rfc, container co, image_file imf
            where co.i_autocode = co_code
            and co.i_autocode = imf.i_foreigncode
            and rf.id = rfc.rf_code                        
    ...
    foreach ($rows as $row) {
        ...
        $newFile = $dest.'/oridir/'.$row->newfile;
        $thumbFile = $dest.'/thumbdir/'.$row->i_autocode.'.jpg';
        $webFile = $dest.'/webdir/'.$row->i_autocode.'.jpg';
        $oldthumbfile =   '/var/www/projects/total-1410-refontedam/back/account/pictures/thumbdir/'.$row->i_autocode.'.jpg';
        ...
        if (!copy($oldFile, $newFile)) {
            ...
            if (file_exists($oldFile)){
                ...
                if (!file_exists($oldthumbfile)){
                    ...
                    $sql = "update restore_files set is_restored = 1 where id = :rfcode";
                    $req = $pdo->prepare($sql);
                    $req->bindValue(':rfcode', $row->rfid, PDO::PARAM_INT);
                    $req->execute();

                    $sql = "update restore_file_co set is_restored = 1 where rf_code = :rfcode and co_code = :cocode";
                    $req = $pdo->prepare($sql);
                    $req->bindValue(':rfcode', $row->rfid, PDO::PARAM_INT);
                    $req->bindValue(':cocode', $row->i_autocode, PDO::PARAM_INT);
                    $req->execute();
                }else{
                    $sql = "insert into restore_ok values (:i_code, true)";
                    $req = $pdo->prepare($sql);
                    $req->bindValue(':i_code', $row->i_autocode, PDO::PARAM_INT);
                    $req->execute();
